...
* **所有表格**；
* **含数学公式（KaTeX）的列表、引用块、段落、标题、定义列表项**
  —— 不含公式的普通段落不会出现按钮，免得满屏都是按钮。

表格、列表、引用块按**整体**导出：悬停在某个单元格、某个列表项、引用块里的某个段落上，
导出的都是最外层那一个（嵌套列表取最外面的 `ol`/`ul`），而不是被悬停的那一项。
...
* **容器整体导出**：表格、列表、引用块是一个整体 —— 悬停在单元格、列表项、引用段落上，
  导出的都是最外层的 `table` / `ol` / `ul` / `blockquote`（引用块连左边的竖线一起）。
  列表里只要有一项含公式，整张列表就可以导出。
...
* `CONTAINER_SELECTOR`：整体导出的容器，默认 `table, ol, ul, blockquote`（限定在 `.markup` 内）。
  想让列表按单项导出，把 `ol, ul` 从这里去掉即可（`li` 已在 `BLOCK_SELECTOR` 里）。
* `BLOCK_SELECTOR`：默认 `p, li, dd, dt, h1..h6`（都限定在 `.markup` 内）。
  想给所有段落都加按钮（不要求含公式），把 `exportTarget()` 里的 `hasMath()` 判断去掉即可。
