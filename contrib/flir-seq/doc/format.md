...
半程两次则 15/21 落在显示精度之外**，见下节。因此查看器**默认**用全程一次。

两种约定都保留：「测温参数」面板顶部的下拉框可以切换（配置项 `defaultModel`，取
`"flir"` 或 `"thermimage"`），切换后整幅图与所有测温点立即重算。需要跟已有的
Thermimage / flirpy 流水线或据此发表过的数据对表时，选 Thermimage 即可复现它们的数字。
...
| --- | --- | --- | --- |
| 半程 τ 乘两次（`thermimage`，Thermimage / flirpy） | 6 / 21 | 0.216 K | **−0.088 K** |
| **全程 τ 用一次（`flir`，默认）** | **21 / 21** | **0.050 K** | **+0.001 K** |

`test/run.mjs` 两条都断言：默认模型必须逐帧命中，而 Thermimage 模型必须在多数统计量上
落在显示精度之外——否则「默认选哪个」就成了没有数据支撑的偏好。
