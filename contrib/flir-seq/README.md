...
* **自动最高/最低点**：每帧自动标出最热（红）与最冷（蓝）像素；鼠标十字光标与测温点标记
  用绿色，与这两个区分开。
...
* **色标控制条**：图像左侧一条竖条，**两端正好是本帧最冷与最热的像素**（不留余量），把
  整帧数据经当前色标映射后画出来——上下两端的纯色带就是被色标**钳位**的那部分像素（白热
  调色板下即纯白与纯黑），中间是被压缩到选定区间的渐变。两个可拖拽滑块直接设定色标上下限，
  **拖不出这两个端点**，拖动即切到「手动」（方向键微调、Shift 粗调、Home/End 直接到端点）。
  条顶部有两个同样大小的按钮：**↕** 回到本帧全范围，**⟲** 回到文件记录的区间，当前已经是
  哪种就置灰哪个。
...
* 大气透过率按 FLIR SDK 的规则取值：文件里的 `estAtmosphericTransmission` 非 0 就直接
  采用，为 0 才按目标距离、湿度与大气温度估算。默认的 **FLIR** 模型按**整段距离**算并只扣
  一次；**Thermimage / flirpy** 模型按半程算再乘两次，读数系统性偏冷（样本上约
  0.1–0.2 °C，远距离更多）。默认选前者的依据是 Thermal Studio 的实测读数，两种约定都
  保留在下拉框里，见 [`doc/format.md`](doc/format.md)。
...
* 色标只影响**画面**：落在区间外的像素按端点颜色画出（不是被隐藏），鼠标悬停照样读出
  真实温度，导出的温度 CSV 也始终是整帧完整数据；导出 PNG 则与屏幕一致，带上钳位效果。
  手动设定的区间按绝对温度记住，切帧时不会跟着自动温标漂走。
