...
* **多帧序列**：`.seq` 通常是多帧，提供逐帧切换、滑块定位与循环播放，并显示每帧
  的采集时间戳。
* **测温参数可调**：发射率、反射表观温度、目标距离、相对湿度、大气温度、红外窗口
  温度与透过率都可现场修改，整幅图与所有测温点立即按新参数重算，一键恢复相机设定。
...
* 大气衰减用的是 FLIR 的标准模型，`ObjectDistance` 取文件里记录的值；如果拍摄
  距离与文件记录不符，请在「测温参数」里改，温度会立刻重算。
